...
describe('GET Session Answers Middleware', function () {
  ...
  it('should send a response with status code 404 and message: "Endpoint not found."', function (done) {
    request(app)
      .get('/intelliq_api/' + 'getsessionanswers')
      .end((err, res) => {
  ...
  it('should send a response with status code 402 and message: "No answers found."', function (done) {
    request(app)
      .get('/intelliq_api/' + 'getsessionanswers/' + 'TT002/' + 'TTTX' + '?format=json')
      .end((err, res) => {
        expect(err).to.be.null;
  ...
  it('should send a response with status code 200', function (done) {
    request(app)
      .get('/intelliq_api/' + 'getsessionanswers/' + 'TT002/' + 'TTTT' + '?format=json')
      .end((err, res) => {
  ...
  it('should have an attribute session', function () {
    expect(serverResponse).to.have.property('session');
  });
  ...
  it('should have an attribute qID inside answers', function () {
    expect(serverResponse.answers[0]).to.have.property('qID');
  });
